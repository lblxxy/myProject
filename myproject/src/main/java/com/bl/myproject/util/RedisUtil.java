...
public class RedisUtil {
     ...
     * 默认redis缓存有效期为20分钟
     */
    private final int defaultExpire= 20 * 60;
    /**
     * 对存储结构为String类型的操作
     */
    ...
    static {
        ...
        //如果赋值为-1，那么就表示无限制，如果已经配置MaxActive个jedis实例，则此时的pool状态为exhausted(耗尽)
        config.setMaxTotal(500);
        //控制一个pool最多多少个idle空闲的jedis实例
        config.setMaxIdle(5);
        //表示当引入jedis实例的最长等待时间，如果超过就会抛出JedisConnectionException
        config.setMaxWaitMillis(1000 * 1000);
        //在引入一个jedis实例时，是否提前进行validate(验证)操作，如果为true，则得到的jedis实例是均可用的
        ...
        //redis未设置了密码：
        //redis的主机IP地址、端口号
        String redisHost = PropertiesUtil.readProperty("redis.host");
        Integer redisPort = Integer.parseInt(PropertiesUtil.readProperty("redis.port"));
     ...
     * @param key jedis Key
     */
    public void defaultExpire(String key){
     ...
     * 通用的保存对象的方法
     * @param key
     * @param value 保存的值，如果是自定义对象注意要实现了序列化接口Serializable接口
     */
    private String set(String key, Object value) {
        Jedis jedis = getJedis();
        String str = null;
        try {
            //序列化是进行编码，二进制。而缓存是保存在内存中，
            // 这里jedis.set(key,byte[]),里面是二进制对象，所以可以强转(byte[]) value,也可以写一个序列化工具，即是对进行编码保存到文件或者流
     ...
     * @param value 保存的值，如果是自定义对象注意要实现了序列化接口Serializable接口
     */
    private String set(String key, Object value,int time) {
        Jedis jedis = getJedis();
        String str = null;
        try {
            //序列化是进行编码，二进制。而缓存是保存在内存中，
            // 这里jedis.set(key,byte[]),里面是二进制字节数组对象,也可以写一个序列化工具，即是对进行编码保存到文件或者流
            str = jedis.set(key.getBytes(), SerializeUtil.serialize(value));
        } catch (Exception e) {
            e.printStackTrace();
        }
        if((time>0) && (Constant.OK).equals(str) ){
            setExpire(key,time);
        }
        recycleJedis(jedis);
        return str;
    ...
    /**
     * 自定义缓存对象
     * @param webName 网站（站点）来源标记
     * @param cacheType 缓存类型，如登录、首页
     * @param key 随意，可以是一串随机数
     * @param value 缓存对象
     * @return
     */
    public String set(String webName,String cacheType,String key,Object value){
        return set(webName+"-"+cacheType+"-"+key,value);
    }
    /**
     * 自定义缓存对象，并设置缓存的有效期
     * @param webName 网站（站点）来源标记
     ...
     * @return
     */
    public String set(String webName,String cacheType,Object value,int time){
        return set(webName+"-"+cacheType,value,time);
    }
    public String set(String webName,String cacheType,String key,Object value,int time){
        return set(webName+"-"+cacheType+"-"+key,value,time);
    }

    /**
     * 与上面对应的根据指定key获取指定缓存对象
     * @param key 缓存key
     * @return 缓存对象
     */
     ...
     * @return
     */
    public Object get(String webName,String cacheType,String key){
        return get(webName+"-"+cacheType+"-"+key);
    }
    public Object get(String webName,String cacheType){
        return get(webName+"-"+cacheType);
    }
    public boolean delete(String webName,String cacheType,String key){
        return delete(webName+"-"+cacheType+"-"+key);
    }
    /*******************************************Sets*******************************************/
    public class Sets {
    ...
    public class Strings {
        ...
        public String set(String key, String value, int time) {
            Jedis jedis = getJedis();
            String str = jedis.setex(key, time, value);
            recycleJedis(jedis);
            return str;
        }
    ...
    public class Lists {
         ...
         * @param key
         * @param value 值
         * @param time  设置过期的过期时间 单位秒
         * @return 状态码
         */
        public String lset(String key, List<Object> value, int time) {
            String result = set(key, value, time);
            return result;
        }
